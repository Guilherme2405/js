function carrinho(){ //mostrando itens no html
    ...
    `
        <li><div class = "">${e.modelo}</div> <div class = "">${e.armazenamento}</div><div class = "">${e.cor}</div> <div class = "">${preco}</div> <div class = ""><button type="submit" class="removeCarrinho" id="${e.id}" value="${index}">Remover do carrinho</button></div> 
    `;
    ...
    document.querySelector('#finalizaCompra').addEventListener('click',(e) => {
        ...
        if(!meuCarrinho || meuCarrinho != ''){
            meuCarrinho.forEach((e,i) => {
                //console.log('carrinho ', i,e.estoque);
                console.log(e.id)

                if(e.estoque > 1){
                    meuCarrinho[i].estoque -= 1;
                }else if (e.estoque === 1) {
                    meuCarrinho[i].estoque -= 1;
                    meuCarrinho[i].disponivel = false;
                } else {
                    meuCarrinho[i].disponivel = false;
                }
                localStorage.setItem('meuCarrinho',JSON.stringify(meuCarrinho));
            ...
            meuCarrinho.forEach(e => {
                ...
                document.querySelector('#itens').innerHTML +=
                  `
                    <li><div class="notaModelo">${e.modelo}</div> <div class="notaArm">${e.armazenamento}</div> <div class="notaCor">${e.cor}</div> <div class="notaPreco">${preco}</div></li>
                 `;
            });
