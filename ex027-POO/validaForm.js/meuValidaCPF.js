...
ValidaCPF.prototype.tiraDigito = function(strCPF) {
    ...
    if(validador == strCPF){
        ...
        const textodiv = document.createTextNode("CPF valido");
        criaDiv.appendChild(textodiv);

        const currentDiv = document.getElementById("div1");
        
        document.querySelector('#cpf').insertAdjacentElement('afterend',criaDiv, currentDiv).classList.add('cpfvalido');
        const cpfinvalido = document.querySelector('.cpfvalido');

    }
    if (validador !== strCPF){
        
        const criaDiv = document.createElement("div");
        const textodiv = document.createTextNode("CPF invalido");
        criaDiv.appendChild(textodiv);

        const currentDiv = document.getElementById("div1");
        document.querySelector('#cpf').insertAdjacentElement('afterend',criaDiv, currentDiv).classList.add('cpfinvalido');
